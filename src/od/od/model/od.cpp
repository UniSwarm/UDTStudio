...
const QMap<QString, QString> &OD::fileInfos() const
{
    return _fileInfos;
}

void OD::setFileInfo(const QString &key, const QString &value)
{
    _fileInfos.insert(key, value);
}

QString OD::fileInfo(const QString &key) const
{
    return _fileInfos.value(key);
}
